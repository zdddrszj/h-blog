...
```
若实现如上功能，首先我们确定的是express是一个函数，执行后返回一个app函数，且有listen方法，我们不如称这个app函数为监听函数。实现代码如下：
```javascript
...
```
若实现如上功能，我们的app监听函数需要实现一个 `get` 方法，该方法可以把本次调用存储在app的路由数组中，当服务启动成功后，监听到匹配的路由时即可调用对应的回调。实现代码如下：
```javascript
let http = require('http')
function createApplication () {
  // app是一个监听函数
...
module.exports = createApplication
```
此时，express的主要功能已经实现，下面来看下如果执行错误通过 `next` 函数参数进行返回的情况。
